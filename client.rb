class Client
    ...
    if attrs["type"] == "subscribe"
      ...
      loop do
        ...
        expect_tag do |name2, attrs2|
          ...
          when "x"
            # in: <x xmlns='vcard-temp:x:update'>
            loop do
              break if next_is_tag_end?
              expect_tag # in: <photo/>
            end
    ...
    when "chat"
      ...
      loop do
        ...
        expect_tag do |name2, attrs2|
          ...
          when "paused", "composing", "active"
            to_client = to_locate attrs["to"]
            to_client.send_typing @user.jid, attrs["type"], attrs["id"], "#{name2}"
          when "html"
  ...
  # <message type='chat' id='purple33cc724f' to='testuser@localhost/Home' from='chris@localhost/Home'>
  #<x xmlns='jabber:x:event'/><paused xmlns='http://jabber.org/protocol/chatstates'/></message>

  def send_typing(from_user, type, id, writing)
    queue_action {
      @xml_output.message "type" => type, "id" => id, "to" => @user.jid, "from" => from_user do
        @xml_output.x "xmlns" => "jabber:x:event" do
          @xml_output.composing if writing == "composing"
        end
